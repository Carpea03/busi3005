...
export function parseQuizIdFromScopedKey(key, prefix) {
  const scopedPrefix = `${prefix}:`;
  if (!String(key || '').startsWith(scopedPrefix)) {
    return null;
  }

  const remainder = key.slice(scopedPrefix.length);
  const separatorIndex = remainder.indexOf(':');
  const quizId = separatorIndex === -1 ? remainder : remainder.slice(0, separatorIndex);
  return quizId || null;
}

export function buildQuizScopedKeyCounts(keys, prefix) {
  return (keys || []).reduce((counts, key) => {
    const quizId = parseQuizIdFromScopedKey(key, prefix);
    if (!quizId) {
      return counts;
    }
...
async function countQuizScopedKeys(pattern, prefix) {
  const redis = await getRedisClient();
  const counts = {};
...
export async function listQuizzesWithStats() {
  const now = new Date();
  const staticQuizzes = getAllQuizzes();
  const redis = await getRedisClient();

  const overrideKeys = staticQuizzes.map((quiz) => statusOverrideKey(quiz.quizId));
  const overrideValues = overrideKeys.length > 0 ? await redis.mGet(overrideKeys) : [];
  const responseCounts = await countQuizScopedKeys('response:*:*', 'response');
  const releaseCounts = await countQuizScopedKeys('release:*:*', 'release');

  const enriched = await Promise.all(
    staticQuizzes.map(async (quiz, index) => {
      const rawOverride = overrideValues[index];
      const override = rawOverride === 'open' || rawOverride === 'closed' ? rawOverride : null;
      const status = computeEffectiveStatus({
      ...
      return {
        ...
        status,
        statusOverride: override,
        openedAt: quiz.releaseAt || null,
        closedAt: quiz.closeAt || null,
        responseCount: responseCounts[quiz.quizId] || 0,
        releaseCount: releaseCounts[quiz.quizId] || 0,
      };
    }),
  );
