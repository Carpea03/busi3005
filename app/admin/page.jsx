      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <button
            ...
            disabled={generatingGroups || submissions.length < 2}
            style={{ padding: '0.5rem 1.25rem', opacity: generatingGroups || submissions.length < 2 ? 0.6 : 1 }}
          >
                  ...
                  <div className="mt-4 pt-4" style={{ borderTop: '1px solid #F0ECF8' }}>
                    <div className="grid sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm" style={{ fontFamily: 'Georgia, serif', color: '#3B3570' }}>{s.format === 'solo' ? 'Solo — n/a' : (s.availability || []).join(', ')}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-sm" style={{ fontFamily: 'Georgia, serif', color: '#3B3570' }}>
                          {s.format === 'solo' ? 'Solo — n/a' : `Deadline approach: ${s.deadlineApproach}/5 · Meeting preference: ${s.meetingPreference || '–'}/5`}
                        </p>
                      </div>
                      {s.peerPreference && (
          ...
          {!groups ? (
            <div className="text-center py-20">
              ...
              <button
                ...
                onClick={handleGenerateGroups}
                disabled={generatingGroups || submissions.length < 2}
                style={{ opacity: submissions.length < 2 ? 0.5 : 1 }}
              >
                {generatingGroups ? '🤖 Generating...' : '🤖 Generate Groups with AI'}
              </button>
              {submissions.length < 2 && (
                <p className="text-xs mt-3" style={{ color: '#9E97C4', fontFamily: 'Georgia, serif' }}>
                  Minimum 2 submissions required (currently {submissions.length})
                </p>
              )}
            </div>
          ) : (
